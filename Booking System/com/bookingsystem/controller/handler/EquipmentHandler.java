...
public class EquipmentHandler implements ActionListener {
	...
	public EquipmentHandler(Handler handler) {
		...
		this.bookingSystemEquipmentPanel = handler.getView()
				.getBookingSystemTabbedPane().getBookingSystemEquipmentPanel();
		this.equipmentTableModel = bookingSystemEquipmentPanel.getTableModel();
		this.currentEquipmentIDBeingHandled = -1;
		this.bookingSystemEquipmentControlPanel = this.bookingSystemEquipmentPanel
				.getBookingSystemEquipmentControlPanel();
		this.bookingSystemAddEquipment = this.bookingSystemEquipmentControlPanel
				.getBookingSystemEquipmenttAddPanel();
		this.bookingSystemEditEquipment = this.bookingSystemEquipmentControlPanel
				.getBookingSystemEquipmenttEditPanel();
		this.bookingSystemRemoveEquipment = this.bookingSystemEquipmentControlPanel
				.getBookingSystemEquipmentRemovePanel();
	}
	...
	public void actionPerformed(ActionEvent eventOccurred) {
		...
		case "Refresh":
			...
			equipmentTableModel.addEquipmentList(IteratorUtils.toList(handler
					.getBookingBusinessLayer().getEquipments().iterator()));
		...
		case "Import":
			...
			try {
				if (jFileChooser.showOpenDialog(bookingSystemEquipmentPanel
					...
					if (jFileChooser.getSelectedFile().getName()
						...
						try (XSSFWorkbook workBook = (XSSFWorkbook) WorkbookFactory
								.create(file)) { // creates a smaller footprint
									...
									if (row.getCell((short) 0).toString() != "") {
										...
										if (handler
											...
											equipmentTableModel
													.addEquipment(importedEquipment);
											equipmentList
													.add(importedEquipment);
											log.setIdPlayedWith(importedEquipment
													.getEquipmentID());

										} else {
											MessageBox
													.errorMessageBox("The equipment '"
															+ importedEquipment
																	.getEquipmentName()
															+ "' already exists.");
										}
		...
		case "Add":
			if (bookingSystemAddEquipment.showDialog() == 0) {
				...
				if (newEquipment.verify()) {
					newEquipment
							.setEquipmentDescription(bookingSystemAddEquipment
					...
					if (handler.getBookingBusinessLayer().getEquipments()
						...
						List<Equipment> list = IteratorUtils.toList(handler
						...
						MessageBox.errorMessageBox("The equipment '"
		...
		case "Edit":
			if (bookingSystemEquipmentPanel.selectedRowCount() > 0) {
				int modelRow = this.bookingSystemEquipmentPanel
						.rowViewIndexToModel(this.bookingSystemEquipmentPanel
								.getSelectedRow());
				System.out.println(bookingSystemEquipmentPanel
						.getValueAt(modelRow, 0));
				this.currentEquipmentIDBeingHandled = (int) bookingSystemEquipmentPanel
						.getValueAt(modelRow, 0);
				if (this.currentEquipmentIDBeingHandled != -1) {
					Equipment oldEquipment = equipmentTableModel
							.getEquipment(this.currentEquipmentIDBeingHandled);
					bookingSystemEditEquipment.setTxtBoxesText(oldEquipment);
					if (this.bookingSystemEditEquipment.showDialog() == 0) {
						Equipment newEquipment = new Equipment(
								this.bookingSystemEditEquipment
										.getEquipmentName());
						newEquipment
								.setEquipmentDescription(this.bookingSystemEditEquipment
										.getEquipmentDescription());
						newEquipment
								.setEquipmentID(this.currentEquipmentIDBeingHandled);
						newEquipment.setEquipmentUsage(oldEquipment
								.getEquipmentUsage());
						if (handler
								.getBookingBusinessLayer()
								.getEquipments()
							...
							handler.getView()
									...
									.addBookingList(
											IteratorUtils.toList(handler
													.getBookingBusinessLayer()
													.iterator()));
							log.setAccountIDCreated(newEquipment
									.getEquipmentID());
						}
					}
				}
				...
				MessageBox
						.errorMessageBox("Please select the equipment you would like to modify.");
			...
			if (bookingSystemEquipmentPanel.selectedRowCount() > 0) {
				...
				if (this.currentEquipmentIDBeingHandled != -1) {
					if (this.bookingSystemRemoveEquipment.showDialog() == 0) {
						int bookingsUsingEquipmentCount = handler
								.getBookingBusinessLayer()
								.getEquipments()
						...
						if (bookingsUsingEquipmentCount > 0) {
							dialogOption = JOptionPane
									.showOptionDialog(
											null,
											"This equipment is being used for "
													+ bookingsUsingEquipmentCount
													+ " bookings, if you want to remove the equipment, all bookings using the equipment will be removed.",
											"Remove Equipment",
											JOptionPane.PLAIN_MESSAGE,
						...
						if (dialogOption == 0
							...
							handler.getBookingBusinessLayer()
									...
									.removeEquipment(
											this.currentEquipmentIDBeingHandled);
							equipmentTableModel
									.removeRows(bookingSystemEquipmentPanel
											.getSelectedRows());
						}
					}
				}
