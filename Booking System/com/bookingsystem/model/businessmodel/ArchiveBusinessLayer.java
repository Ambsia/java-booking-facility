...
public class ArchiveBusinessLayer extends BusinessLayer {
        ...
        if (getDatabaseConnector().isConnected()) {
            if (getDatabaseConnector().isConnectionClosed()) {
                ...
                try (ResultSet rs = getDatabaseConnector().executeQuery()) {
                    ...
                    MessageBox
                            .errorMessageBox("There was an issue while we were trying to get total bookings made!\n"
        ...
        if (getDatabaseConnector().isConnected()) {
            if (getDatabaseConnector().isConnectionClosed()) {
                ...
                try (CallableStatement callableStatement = getDatabaseConnector()
                    ...
                    MessageBox
                            .errorMessageBox("There was an issue while we were trying to retrieve the busiest hours!\n"
                                    + "Does this make any sense to you.."
                                    + e.toString() + "?");
                }
            }
        ...
        if (getDatabaseConnector().isConnected()) {
            if (getDatabaseConnector().isConnectionClosed()) {
                ...
                try (ResultSet rs = getDatabaseConnector().executeQuery()) {
                    ...
                    MessageBox
                            .errorMessageBox("There was an issue while we were trying to get the total amount of bookings completed!\n"
                                    + "Does this make any sense to you.."
                                    + e.toString() + "?");
                }
            }
        ...
        if (getDatabaseConnector().isConnected()) {
            if (getDatabaseConnector().isConnectionClosed()) {
                ...
                try (CallableStatement callableStatement = getDatabaseConnector()
                    ...
                    MessageBox
                            .errorMessageBox("There was an issue while we were trying to get the equipment that is most used!\n"
                                    + "Does this make any sense to you.."
                                    + e.toString() + "?");
                }
            }
            ...
            if (getDatabaseConnector().isConnectionClosed()) {
                ...
                try (CallableStatement callableStatement = getDatabaseConnector()
                    ...
                    try (ResultSet rs = getDatabaseConnector().executeQuery()) {
                        int i = 0;
                        while (rs.next() && i < 5) {
                            mostUsedLocation.add("- " + rs.getString(1) + " ("
                                    + rs.getInt(2) + ")");
                            i++;
                        }
                    }
                    ...
                    MessageBox
                            .errorMessageBox("There was an issue while we were trying to get the location that is most used!\n"
                                    + "Does this make any sense to you.."
                                    + e.toString() + "?");
                }
            }
    ...
    public ArrayList<String> getStaffMemberWithTheMostBookingsMade() {
        ...
        if (getDatabaseConnector().isConnected()) {
            if (getDatabaseConnector().isConnectionClosed()) {
                getDatabaseConnector().createNewCallableStatement(
                        "{CALL spGetMostBookingsMadeBy}");
                try (CallableStatement callableStatement = getDatabaseConnector()
                        .getCallableStatement()) {
                    try (ResultSet rs = getDatabaseConnector().executeQuery()) {
                        int i = 0;
                        while (rs.next() && i < 5) {
                            staffMemberWithTheMostBookingsMadeList.add("- "
                                    + rs.getString(1).trim() + " ("
                                    + rs.getInt(2) + ")");
                            i++;
                        }
                    ...
                    MessageBox
                            .errorMessageBox("There was an issue while we were trying to get the staff member with the most bookings made!\n"
                                    + "Does this make any sense to you.."
                                    + e.toString() + "?");
                }
            }
        ...
        if (getDatabaseConnector().isConnected()) {
            if (getDatabaseConnector().isConnectionClosed()) {
                ...
                try (ResultSet rs = getDatabaseConnector().executeQuery()) {
                    ...
                    MessageBox
                            .errorMessageBox("There was an issue while we were trying to get the total days booked!\n"
                                    + "Does this make any sense to you.."
                                    + e.toString() + "?");
                }
            }
