 ...
 * Author: [Alex]
 */
public class AccountManagementBusinessLayer extends BusinessLayer implements
        Iterable<Account> {
        ...
        if (getDatabaseConnector().isConnected()) {
            if (getDatabaseConnector().isConnectionClosed()) {
                getDatabaseConnector().createNewCallableStatement(
                        "{CALL spCheckForDuplicateUsername(?,?)}");
                try (CallableStatement csCheckForDupeName = getDatabaseConnector()
                    ...
                    if (csCheckForDupeName.getInt(2) == 0) {
                        ...
                        MessageBox
                                .errorMessageBox("There is already an account with the username '"
                                        + account.getUsername() + "'");
                    }
                } catch (SQLException e) {
                    MessageBox
                            .errorMessageBox("There was an issue while adding an account.\n"
            ...
            if (getDatabaseConnector().isConnected()) {
                if (getDatabaseConnector().isConnectionClosed()) {
                    ...
                    try (CallableStatement callableStatement = getDatabaseConnector()
                        ...
                        MessageBox
                                .errorMessageBox("There was an issue while retrieving accounts.\n"
                                        + "Does this make any sense to you.."
                                        + e.toString() + "?");
                    }
                }
    ...
    public void getAllAccounts() {
        ...
        if (getDatabaseConnector().isConnected()) {
            if (getDatabaseConnector().isConnectionClosed()) {
                getDatabaseConnector().createNewCallableStatement(
                        "{CALL spGetAllAccounts}");
                try (ResultSet rs = getDatabaseConnector().executeQuery()) {
                    while (rs.next()) {
                        this.accountList.add(new Account(rs.getInt(1), rs
                                .getInt(4), rs.getString(2), rs.getString(3)));
                    }
                } catch (SQLException e) {
                    MessageBox
                            .errorMessageBox("There was an issue while retrieving accounts.\n"
    ...
    // --Commented out by Inspection (21/06/2015 00:45):public void
    // editAccount(int accountID) { }
    ...
    // if (currentIndexInAccountList >= 0 && currentIndexInAccountList <=
    // accountList.size()) {
    ...
    public void setCurrentIndexOfAccountInList(int indexOfBookingInList) {
        try {
            if (indexOfBookingInList >= 0
                    && indexOfBookingInList <= accountList.size()) {
