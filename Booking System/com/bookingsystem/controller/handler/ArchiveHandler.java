...
public class ArchiveHandler implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			case "View Statistics":
				...
				if (!handler.getBookingBusinessLayer().getArchivedBookings()
					...
					int totalDaysBooked = handler.getArchiveBusinessLayer()
							.getTotalDaysBooked();
					int totalBookingsCompleted = handler
							.getArchiveBusinessLayer()
							.getTotalBookingsCompleted();

					int averageBookingsADay = totalDaysBooked != 0 ? (totalBookings / totalDaysBooked)
							: 0;
					Date[] busiestHours = handler.getArchiveBusinessLayer()
							.getBusiestHours();

					String mostUsedEquipmentString = "";
					for (Equipment equipment : handler
					...
					ArrayList<String> mostUsedLocationList = handler
							.getArchiveBusinessLayer().getMostUsedLocation();
					for (String s : mostUsedLocationList) {
						mostUsedLocationString += s + "\n";
					}

					String mostBookingsHeldByString = "";
					ArrayList<String> mostBookingsHeldByList = handler
							.getArchiveBusinessLayer()
					...
					bookingSystemArchive
							.getUiBookingSystemArchiveViewPanel()
							.setJTextArea(
									"Booking Statistical view from dd/mm/yyyy to dd/mm/yyyy:\n"
											+ "\n" + "Total Bookings Made: "
											+ totalBookings
											+ "\n"
											+ "\n"
											+ "Total Junior School Bookings: "
											+ juniorBookings
											+ "\n"
											+ "\n"
											+ "Total Senior School Bookings: "
											+ seniorBookings
											+ "\n"
											+ "\n"
											+ "Total Days Booked: "
											...
											+ "\n"
											+ "\n"
											+ "Average Bookings Each Day:  "
											+ averageBookingsADay
											+ "\n"
											+ "\n"
											+ "Busiest Hours:   "
											+ BOOKING_TIME_FORMAT
													.format(busiestHours[0])
											+ "-"
											+ BOOKING_TIME_FORMAT
													.format(busiestHours[1])
											+ "\n"
											+ "\n"
			...
			MessageBox
					.errorMessageBox("An error occurred whilst loading archives, please reload all bookings in the main booking panel.");
		}
	}
}
