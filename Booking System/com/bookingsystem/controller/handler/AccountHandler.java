	...
	public void actionPerformed(ActionEvent e) {
		...
		case "Change Password":
			if (bookingSystemAdminPanel.selectedRowCount() > 0) {
				...
				System.out.println(handler.getAccountBusinessLayer()
						.getAccountLoggedIn().getUserLevel());
				if (account.getUserLevel() < 3
						&& handler.getAccountBusinessLayer()
								.getAccountLoggedIn().getUserLevel() >= 3
						|| this.currentAccountIDBeingProcessed == handler
								.getAccountBusinessLayer().getAccountLoggedIn()
								.getUserID()
						|| handler.getAccountBusinessLayer()
								.getAccountLoggedIn().getUserLevel() >= 5) {
					bookingSystemChangePasswordPanel
							.setLblUsernameText("Changing password for: "
									+ account.getUsername());
								...
								if (handler
										...
										.changePassword(
												...
												accountTableModel
														...
														.getUsername(),
												password)) {
									if (this.currentAccountIDBeingProcessed != handler
											.getAccountBusinessLayer()
